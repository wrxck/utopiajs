...
export function computed<T>(fn: () => T): ReadonlySignal<T> {
  return makeCallable(new ComputedNode<T>(fn));
}
...
class EffectNode implements Subscriber {
  ...
  notify(): void {
    ...
    // Every notification originates inside SignalNode._write (directly or
    // via ComputedNode.notify propagation), which always holds batchDepth > 0
    // while notifying — so execution is always deferred to the flush that
    // runs when the outermost write/batch completes.
