...
export const EmailButton = {
  ...
  render: (ctx: any) => {
    ...
    setAttr(td, 'style', `background-color: ${ctx.color}; border-radius: ${ctx.borderRadius}; padding: 12px 24px`);

    const a = createElement('a');
    setAttr(a, 'href', ctx.href);
    setAttr(a, 'style', `color: ${ctx.textColor}; text-decoration: none; font-size: 16px; font-weight: bold; display: inline-block`);
    appendChild(a, createTextNode(ctx.text));
